...
module.exports = function addToCollection (route, options) {
  ...
  return function (request, reply) {
    ...
    // ...a primary key:
    var supposedChildPk = actionUtil.parsePk(request, options, true);
    if (supposedChildPk) {
      ...
      child = actionUtil.parseValues(request, options, true);
    ...
    Async.auto({
      ...
      parent: function (cb) {
        ...
        Model.findOne(parentPk).exec(function foundParent(err, parentRecord) {
          ...
          if (!actionUtil.validOwnership(parentRecord, false, request, options)) return cb(Boom.unauthorized());
      ...
      actualChild: ['parent', function(cb) {
        ...
        if (child[childPkAttr]) {
          ...
          ChildModel.findOne(child[childPkAttr]).exec(function foundChild(err, childRecord) {
            ...
            if (!actionUtil.validOwnership(childRecord, true, request, options))
    ...
    function readyToSave (err, asyncResults) {
      ...
      asyncResults.parent.save(function saved(err) {
        ...
        if (createdChild) {
          ...
          return reply(asyncResults.actualChild).created();
        } else {
