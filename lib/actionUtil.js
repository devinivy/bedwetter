...
module.exports = {
  ...
  parseValues: function (request, options, child) {
    ...
    if (options.actAsUser &&
      ...
      if (_.isUndefined(values[options.ownerAttr]) || _.isNull(values[options.ownerAttr])) {
        
        values[options.ownerAttr] = this.getUserId(request, options);
  ...
  getUserId: function(request, options) {
    ...
    Hoek.assert(_.isObject(request.auth.credentials), 'Authentication credentials were not set.')
